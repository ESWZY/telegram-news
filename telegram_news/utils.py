...
def save_compressed_video(video_full_path, size_upper_bound, two_pass=True, filename_suffix='1'):
    ...
    try:
        ...
        video_bitrate = ((size_upper_bound * 1024 * 8) / (1.048576 * duration)) - audio_bitrate
        # If audio size is bigger than `size_upper_bound`
        if video_bitrate < 0:
            audio_bitrate *= (size_upper_bound * 1024) / os.path.getsize(video_full_path)
            video_bitrate = ((size_upper_bound * 1024 * 8) / (1.073741824 * duration)) - audio_bitrate
